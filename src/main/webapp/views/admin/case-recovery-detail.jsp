					...
					<table class="table">
						...
						<thead>
							...
							<tr>
								<th>操作</th>
								<td colspan="2">
									<button type="button" onclick="unRecoverCase();" class="btn btn-info">恢复正常</button>
								</td>
								<td colspan="2">
									<a href="#" onclick="deleteTip();" class="btn btn-info">彻底删除</a>
								</td>
								<td colspan="2">
...
<script>
	...
	function unRecoverCase(){
		$.ajax({
			url : "<c:url value="/admin/unRecoverCase"/>?id=${caseInfo.id }",
			type : "GET",
			success : function(data) {
				if (data.status == "1") {
					alert("处理成功");
